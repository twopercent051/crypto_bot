...
async def admin_start_msg(message: Message):
    ...
    await message.answer(text, reply_markup=main_admin_keyboard)
...
async def admin_start_clb(callback: CallbackQuery):
    ...
    await callback.message.answer(text, reply_markup=main_admin_keyboard)
...
async def admin_get_course(callback: CallbackQuery):
    ...
    await callback.message.delete()
    await bot.answer_callback_query(callback.id)
    await callback.message.answer('\n'.join(text), reply_markup=coin_keyboard)
...
async def admin_edit_buy_price(callback: CallbackQuery, state: FSMContext):
    ...
    price = price_cor[2]
    text = [
        f'Сейчас цена покупки <b>{coin}</b> составляет {price} ₽. Введите нову цену или нажмите ПРОПУСТИТЬ',
    ...
    await callback.message.delete()
    await bot.answer_callback_query(callback.id)
    await callback.message.answer('\n'.join(text), reply_markup=miss_keyboard)
...
async def admin_edit_sell_price_msg(message: Message, state: FSMContext):
    try:
        ...
        price_sell_cor = await get_course(coin)
        ...
        await message.delete()
        await message.answer(' '.join(text), reply_markup=miss_keyboard)
    except:
        text = 'Вы ввели не число. Попробуйте снова или отмените ввод'
        await message.delete()
        await message.answer(text, reply_markup=miss_keyboard)
...
async def admin_edit_sell_price_clb(callback: CallbackQuery, state: FSMContext):
    ...
    await FSMEditPrice.sell_price.set()
    await callback.message.delete()
    await callback.message.answer(' '.join(text), reply_markup=miss_keyboard)
    await bot.answer_callback_query(callback.id)
...
async def admin_edit_price_finish_clb(callback: CallbackQuery):
    ...
    await FSMEditPrice.sell_price.set()
    await callback.message.delete()
    await callback.message.answer(' '.join(text), reply_markup=home_keyboard)
    await bot.answer_callback_query(callback.id)
...
async def get_offers(callback: CallbackQuery):
    ...
    await bot.answer_callback_query(callback.id)
    await callback.message.answer(text, reply_markup=offer_type_keyboard)
...
async def get_statistic(callback: CallbackQuery):
    ...
    for day in days:
        ...
        await callback.message.answer('\n'.join(day_text), reply_markup=home_keyboard)
        await bot.answer_callback_query(callback.id)
...
def register_admin(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(admin_get_course, lambda x: x.data == 'edit_course', state='*',
                                       chat_id=admin_group)
    dp.register_callback_query_handler(admin_edit_buy_price, lambda x: x.data.split(':')[0] == 'coin', state='*',
                                       chat_id=admin_group)
